...
#include "linear_mpc_controller/regulated_pure_pursuit_controller.hpp"
...
void LinearMPCController::cleanup()
{
  RCLCPP_INFO(
    ...
    " regulated_pure_pursuit_controller::LinearMPCController",
    plugin_name_.c_str());
...
std::unique_ptr<geometry_msgs::msg::PointStamped> LinearMPCController::createCarrotMsg(
...
double LinearMPCController::getLookAheadDistance(
...
geometry_msgs::msg::TwistStamped LinearMPCController::computeVelocityCommands(
  ...
  gestelt_core::GoalChecker * goal_checker)
{
  ...
  if (use_collision_detection_ && isCollisionImminent(pose, linear_vel, angular_vel, carrot_dist)) {
    throw gestelt_core::PlannerException("LinearMPCController detected collision ahead!");
  }
...
bool LinearMPCController::shouldRotateToPath(
...
bool LinearMPCController::shouldRotateToGoalHeading(
  const geometry_msgs::msg::PoseStamped & carrot_pose)
{
...
void LinearMPCController::rotateToHeading(
...
geometry_msgs::msg::Point LinearMPCController::circleSegmentIntersection(
...
geometry_msgs::msg::PoseStamped LinearMPCController::getLookAheadPoint(
...
bool LinearMPCController::isCollisionImminent(
...
bool LinearMPCController::inCollision(
  const double & x,
  const double & y,
  const double & theta)
{
...
double LinearMPCController::costAtPose(const double & x, const double & y)
{
  unsigned int mx, my;
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    ...
    throw gestelt_core::PlannerException(
            "LinearMPCController: Dimensions of the costmap are too small "
...
double LinearMPCController::approachVelocityScalingFactor(
...
void LinearMPCController::applyApproachVelocityScaling(
...
void LinearMPCController::applyConstraints(
...
double LinearMPCController::findVelocitySignChange(
  const nav_msgs::msg::Path & transformed_plan)
{
...
double LinearMPCController::getCostmapMaxExtent() const
{
