#include "gestelt_planner/planner_server.hpp"
...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  // Launch a thread to run the occ_map node
  occ_map_thread_ = std::make_unique<nav2_util::NodeThread>(occ_map_);

  tf_ = occ_map_->getTfBuffer();
  ...
  geometry_msgs::msg::PoseStamped & start)
{
  if (goal->use_start) {
    start = goal->start;
  } else if (!occ_map_->getRobotPose(start)) {
    return false;
  }
...
{
  if (!occ_map_->transformPoseToGlobalFrame(curr_start, curr_start) ||
    !occ_map_->transformPoseToGlobalFrame(curr_goal, curr_goal))
  {
    return false;
  }
  ...
  std::function<bool()> cancel_checker)
{
  ...
  auto start_eig = Eigen::Vector3d(start.pose.position.x, start.pose.position.y, start.pose.position.z);
  auto goal_eig = Eigen::Vector3d(goal.pose.position.x, goal.pose.position.y, goal.pose.position.z);
  ...
  if (planners_.find(planner_id) != planners_.end()) {
    ...
    if (planners_.size() == 1 && planner_id.empty()) {
      ...
      return planners_[planners_.begin()->first]->createPlan(start_eig, goal_eig, cancel_checker);
    } else {
      RCLCPP_ERROR(
        get_logger(), "planner %s is not a valid planner. "
        "Planner names are: %s", planner_id.c_str(),
        planner_ids_concat_.c_str());
      throw gestelt_core::InvalidPlanner("Planner id " + planner_id + " is invalid");
    }
  }
  ...
  std::shared_ptr<nav2_msgs::srv::IsPathValid::Response> response)
{
  ...
  if (occ_map_->getRobotPose(current_pose)) {
    ...
    bool use_radius = occ_map_->getUseRadius();

    unsigned int cost = occ_map::FREE_SPACE;
    for (unsigned int i = closest_point_index; i < request->path.poses.size(); ++i) {
      ...
      if (use_radius) {
        ...
        if (occ_map_->worldToMap(Eigen::Vector3d(position.x, position.y, position.z), pos_map)) {
          cost = occ_map_->getCost(pos_map);
        } else {
          cost = occ_map::LETHAL_OBSTACLE;
        }
      } else {
      ...
      if (use_radius &&
        (cost == occ_map::LETHAL_OBSTACLE ||
        cost == occ_map::INSCRIBED_INFLATED_OBSTACLE))
      {
        response->is_valid = false;
        break;
      } else if (cost == occ_map::LETHAL_OBSTACLE) {
        response->is_valid = false;
        break;
      }
    }
...
RCLCPP_COMPONENTS_REGISTER_NODE(gestelt_planner::PlannerServer)
