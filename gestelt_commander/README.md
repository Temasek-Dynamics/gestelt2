...
from gestelt_commander.robot_navigator import BasicNavigator
import rclpy
...
Make sure you have set the model directory of turtlebot3 simulation and aws warehouse world to the `GAZEBO_MODEL_PATH`. There are 2 main ways to run the demos of the `gestelt_commander` API.

### Automatically

The main benefit of this is automatically showing the above demonstrations in a single command for the default robot model and world. This will make use of Nav2's default robot and parameters set out in the main simulation launch file in `gestelt_bringup`.

``` bash
# Launch the launch file for the demo / example
ros2 launch gestelt_commander  security_demo_launch.py
```

This will bring up the robot in the AWS Warehouse in a reasonable position, launch the autonomy script, and complete some task to demonstrate the `gestelt_commander` API.
...
ros2 launch gestelt_bringup tb3_simulation_launch.py world:=/path/to/aws_robomaker_small_warehouse_world/.world map:=/path/to/aws_robomaker_small_warehouse_world/.yaml

# Terminal 2: launch your autonomy / application demo or example. For example
ros2 run gestelt_commander demo_security
```
...
The `gestelt_commander` has a few examples to highlight the API functions available to you as a user:
...
The `gestelt_commander` has a few demonstrations to highlight a couple of simple autonomy applications you can build using the `gestelt_commander` API:
