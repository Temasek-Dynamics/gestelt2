        ...
        for (int dx = -5; dx <= 5; dx++)
        {
          for (int dy = -5; dy <= 5; dy++)
          {
            ...
            const Eigen::Vector3d goal_temp_check = Eigen::Vector3d(
              ...
              // map_goal.z()
              map_goal.z() + ((double)dz * 0.2)
            );

            if (!occ_map_->inGlobalMap(goal_temp_check))
            {
              throw gestelt_core::GoalOutsideMapBounds(
                "Checked goal(Map frame) Coordinates of(" + std::to_string(goal_temp_check(0)) + ", " +
                std::to_string(goal_temp_check(1)) + ", " + std::to_string(goal_temp_check(2)) + 
                ") was outside bounds");
            }
